...
import sys
import pandas as pd
import Models.mhcvision_pred
import Models.rf_pred
import sklearn
...
def run_models(allele, prediction_tool, input_file):
    ...
    Models.rf_pred.immune_pred()  ## return peptide and immune prob
    print('The immunogenicity prediction is running...')
    """
    run MHCVision
    """
    ...
    data = Models.mhcvision_pred.convert_score_for_beta(input_file)  # converted IC50
    print('The parameter estimation is running...')
    model = Models.mhcvision_pred.BMM(data, allele, hla_parameter_range, input_file,)
    model.initialisation()
    model.termination()
    est_fdr = Models.mhcvision_pred.FDR('beta_parameter.csv', data, allele, input_file)
...
# if everything has been checked out, run the models
    ...
    fdr_pred['Immunogenic probability'] = immune_prob
    final_prob = immune_prob*true_binding_prob
    fdr_pred['Final probability'] = final_prob
    # write final output
    fdr_pred.to_csv(output_file, index=False)
